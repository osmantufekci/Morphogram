            ...
            if let url = outputURL as CFURL? {
                if let destination = CGImageDestinationCreateWithURL(url, UTType.gif.identifier as CFString, images.count, nil) {
                    ...
                    while(!chosenImages.isEmpty) {
                        let img = chosenImages.remove(at: 0)
                        print("Images:", img, "framecount:", frameCount)
                        guard var image = ImageManager.shared.loadImage(fileName: img) else { continue }
                        autoreleasepool {
                            if let fixedImage = image.fixImageOrientation(by: outputSize.size) { image = fixedImage }
                        }
